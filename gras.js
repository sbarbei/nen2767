(function(){
  ...
  // ===== Hoofdactie =====
  function actSjabloon(eind,gev,groeiGev,overlapGev,begin,groeiBegin,overlapBegin) {
    if(eind) {
      clickId("newbutton");
  ...
  m.innerHTML = `
    ...
    <table style="width:100%;margin-top:18px;font-size:15px;">
      <tr>
        <td><b>Bedekkingsgraad (Serieus) Eindstadium</b></td>
        <td><input type='number' id='eind' min='0' max='100' style='width:60px;' placeholder="%" autocomplete="off"></td>
        <td></td>
      </tr>
      <tr>
        <td><b>Bedekkingsgraad (Serieus) Gevorderd stadium</b></td>
        <td><input type='number' id='gevorderd' min='0' max='100' style='width:60px;' placeholder="%" autocomplete="off"></td>
        <td></td>
      </tr>
      <tr>
        <td><b>Groeiontwikkeling (Serieus) Gevorderd stadium</b></td>
        <td><input type='number' id='groeig' min='0' max='100' style='width:60px;' placeholder="%" autocomplete="off"></td>
        <td><label style="font-size:13px;"><input type="checkbox" id="overlapg" checked style="margin-right:4px;">Overlap</label></td>
      </tr>
      <tr>
        <td><b>Bedekkingsgraad (Serieus) Beginstadium</b></td>
        <td><input type='number' id='begin' min='0' max='100' style='width:60px;' placeholder="%" autocomplete="off"></td>
        <td></td>
      </tr>
      <tr>
        <td><b>Groeiontwikkeling (Serieus) Beginstadium</b></td>
        <td><input type='number' id='groeib' min='0' max='100' style='width:60px;' placeholder="%" autocomplete="off"></td>
        <td><label style="font-size:13px;"><input type="checkbox" id="overlapb" checked style="margin-right:4px;">Overlap</label></td>
      </tr>
    </table>
    <div style='margin-top:12px;margin-bottom:8px;font-size:12px;color:#666;'>Vul de gewenste percentages per rij in.<br>Standaard staan de percentages van groeiontwikkeling gelijk aan bedekkingsgraad (maar je mag ze aanpassen).</div>
    <div id="zonderGebreken" style="margin-top:10px;font-size:15px;font-weight:bold;">Zonder gebreken: 100%</div>
  `;
  ...
  // Koppel update aan alle inputvelden
  ["eind", "gevorderd", "groeig", "begin", "groeib"].forEach(function(id){
    m.querySelector("#"+id).addEventListener("input", updateZonderGebreken);
  });
  // Bij openen direct de juiste waarde tonen:
  updateZonderGebreken();
